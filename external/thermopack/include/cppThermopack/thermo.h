...
// thermo.h 中，class Thermo { public: ... };

void getCriticalParams(std::vector<double>& Tc,
                       std::vector<double>& Pc,
                       std::vector<double>& omega) const
{
    // 确保 Thermopack 当前激活的是这个 EOS 对象
    activate();  // 你已有的函数，内部会调用 thermopack_var_activate_model

    Tc.assign(nc, 0.0);
    Pc.assign(nc, 0.0);
    omega.assign(nc, 0.0);

    for (int j = 0; j < static_cast<int>(nc); ++j) {
        int idx = j + 1; // Fortran 组分是 1-based
        double tci = 0.0, pci = 0.0, oi = 0.0;

        // 用extern.h中手动声明的函数接口，函数符号与 thermopack.h 中一致
        thermopack_getcriticalparam_c(&idx, &tci, &pci, &oi);

        Tc[j]    = tci;
        Pc[j]    = pci;
        omega[j] = oi;   // acentric factor
    }
}
...
void wilsonK(double T, double P, std::vector<double>& K) const
{
    ...
    double TT = T;
    double PP = P;
    thermopack_wilsonk_c(&TT, &PP, K.data());
}
